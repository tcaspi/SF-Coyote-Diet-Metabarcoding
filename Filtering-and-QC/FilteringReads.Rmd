...
For batch 1, sequences comprising <0.001% of the total reads have already been removed. For batch 2, start by removing these. Then for both batches, drop sequences flagged for removal due to low percent identity or query cover.
...
seqs2 <- seqs2 %>% 
  filter(Flag != "x") %>% 
  filter(Perc.Total >= 0.001) %>% # remove ASVs <0.001% of the total reads
...
# Separate control from samples

```{r}
# Batch 1
batch1_ctrls <- seqs1_count %>% select(-c("S20_3645":"S21_1194"))
batch1_sams <- seqs1_count %>% select(1, "S20_3645":"S21_1194")

# Batch 1
batch2_ctrls <- seqs2_count %>% select(-c("RS20_3767":"S22_1016"))
batch2_sams <- seqs2_count %>% select(1, "RS20_3767":"S22_1016")
```
...
1.  Removing samples collected outside the study region of San Francisco
2.  Removing any samples with a known non-coyote origin (e.g., Cytb, genotyping, or intentionally collected dog samples)
5.  Removing samples that generated no *Canis* spp. reads
6.  Removing duplicate samples (PCR replication)
...
clean.data <- function(x) {left_join(x, metadata, by = "SampleID") %>%
    filter(Area == "Urban") %>%  # remove samples not in SF
