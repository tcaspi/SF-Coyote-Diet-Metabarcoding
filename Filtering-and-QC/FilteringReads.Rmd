...
# Create data frames with filtered read counts for specialization analysis

To create data frames of (filtered) raw read counts, convert proportions back to raw reads except for reads that represent <0.5% of the total and rare and/or small-bodied prey that is present in low proportions.

```{r}
# Define function to remove diet items representing <0.5% of reads within a scat
convert.to.raw <- function(proportion, total) {
  return(proportion * total)
}

# Extract total read columns
totals_batch1 <- rm_low_batch1 %>% select(c(SampleID, Total))
totals_batch2 <- rm_low_batch2 %>% select(c(SampleID, Total))


# Convert proportions back to raw reads
filtered_rawseqs_batch1 <- left_join(totals_batch1, rm.rare_batch1, by="SampleID") %>% 
  mutate(across(where(is.numeric) & !any_of("Total"), ~ convert.to.raw(., Total))) %>% 
  select(-c(Total))

filtered_rawseqs_batch2 <- left_join(totals_batch2, rm.rare_batch2, by="SampleID") %>% 
  mutate(across(where(is.numeric) & !any_of("Total"), ~ convert.to.raw(., Total))) %>% 
  select(-c(Total))

# Combine data frames
filtered_rawseqs_combined <- plyr::rbind.fill(filtered_rawseqs_batch1, filtered_rawseqs_batch2) %>% replace(is.na(.), 0) # replace NAs with zeros
  
# Identify columns where all values are zeros
zero_columns_rawseqs <- sapply(filtered_rawseqs_combined[, -1], function(col) all(col == 0))

# Remove columns where all values are zeros
filtered_rawseqs <- filtered_rawseqs_combined[, c(TRUE, !zero_columns_rawseqs)]

# Reorder alphabetically
new.order.rawseqs <- sort(colnames(filtered_rawseqs[,2:ncol(filtered_rawseqs)])) # generate alphabetical list of diet items
filtered_rawseqs <- filtered_rawseqs[,c("SampleID", new.order.rawseqs)] #reorder species columns to be alphabetical
```
...
input <- list(func.RRA, func.FOO, sp.RRA, sp.FOO, filtered_rawseqs)
...
filtered_rawseqs.output <- results[[5]]
...
filtered_rawseqs.output <- filtered_rawseqs.output %>% select(-c("Replicate":"Cytb_Sp"))
...
rawseqs.clean <- filtered_rawseqs.output[, c(TRUE, !zero_columns)]
```
...
write.csv(rawseqs.clean, "Filtering-and-QC/raw.seqs.clean.csv")
```
